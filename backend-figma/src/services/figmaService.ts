/**
 * Figma API Service
 * All Figma API interactions go through this service
 */

import { figmaConfig, getFigmaHeaders } from '../config/figma';
...
export async function getFigmaFile(fileKey: string): Promise<FigmaFile> {
  try {
    ...
    const timeoutId = setTimeout(() => controller.abort(), figmaConfig.timeout);
...
export async function getFigmaComments(fileKey: string): Promise<{ comments: any[] }> {
  try {
    ...
    const response = await fetch(
      `${figmaConfig.baseUrl}/files/${fileKey}/comments`,
      {
        headers: getFigmaHeaders(),
        signal: controller.signal,
      }
...
export async function healthCheck(): Promise<{ status: 'ok' | 'error'; message: string }> {
  try {
    // Try to fetch a public file or user info
    const response = await fetch(`${figmaConfig.baseUrl}/me`, {
      headers: getFigmaHeaders(),
    });
